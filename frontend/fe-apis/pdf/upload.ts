/**
 * fe-apis/pdf/upload.ts
 *
 * Business logic for POST /api/pdf/upload.
 * Accepts a multipart PDF file, extracts text, chunks it, and persists to SQLite.
 */
...
} from '@/modules/swipe-pdf-reader/utils/pdfProcessor';
import { getSQLiteDatabase } from '../_shared/db';

export async function handleUploadPdf(request: Request): Promise<NextResponse> {
  try {
    ...
    // Persist to SQLite if the DB is accessible on this deployment
    ...
    if (res) {
      ...
      try {
        ...
        console.warn('[fe-api/pdf/upload] SQLite write fallback:', err);
      }
    }
