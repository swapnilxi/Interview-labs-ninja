...
import { portfolioPreviewHtml, resumePreviewHtml, type PreviewSection } from './templatePreview';
...
export default function TemplateDesigner({ kind, template, profiles, initialProfileId, onClose, onSaved }: {
  kind: ViewKind;
  template?: CareerTemplate | null;
  /** Profiles the preview can pull real data from — when provided, a "Preview
   * with" dropdown lets the user pick one instead of seeing generic sample
   * copy (see templatePreview.ts). */
  profiles?: Resume[];
  ...
  const [previewProfileId, setPreviewProfileId] = useState(initialProfileId || profiles?.[0]?.id || '');
  const [previewSections, setPreviewSections] = useState<PreviewSection[] | undefined>(undefined);

  useEffect(() => {
    if (!previewProfileId) { setPreviewSections(undefined); return; }
    let cancelled = false;
    careerService.getProfile(previewProfileId).then((p) => { if (!cancelled) setPreviewSections(p.sections as PreviewSection[] | undefined); }).catch(() => {});
  ...
  const previewSrc = useMemo(
    () => (isResume ? resumePreviewHtml(spec as ResumeTemplateSpec, previewSections) : portfolioPreviewHtml(spec as PortfolioTemplateSpec, previewSections)),
    [isResume, spec, previewSections],
  );

  const [savedTemplate, setSavedTemplate] = useState<CareerTemplate | null>(template || null);
  const persist = async (): Promise<CareerTemplate> => {
    const saved = savedTemplate
      ? await templatesService.update(savedTemplate.id, { name: name.trim() || 'Untitled', spec })
  ...
  // Publishing a live site starts one level up (a "view" built from a
  // profile + this template), not from the abstract template itself — this
  // is the direct shortcut there, so "design a template" and "publish it"
  // aren't separated by a trip back through the Templates gallery.
  const [usingTemplate, setUsingTemplate] = useState(false);
  const useTemplate = async () => {
    if (!previewProfileId) {
      setError('Pick a profile to preview with first');
      return;
    }
    ...
    try {
      ...
      const view = await viewsService.create({ profileId: previewProfileId, kind, template: saved.id });
      router.push(`/career/view/${view.id}`);
    } catch (e: any) {
      setError(e?.message || 'Could not create from this template');
    ...
    <div className="fixed inset-0 z-[300] bg-background flex flex-col">
      ...
      <div className="h-[56px] flex items-center gap-2 px-4 border-b border-border bg-card flex-shrink-0">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button onClick={save} disabled={saving || usingTemplate} className="px-4 py-1.5 rounded-lg border border-border text-xs font-semibold text-foreground hover:bg-muted disabled:opacity-50 inline-flex items-center gap-1.5"><Icon name="CheckIcon" size={14} /> {saving ? 'Saving…' : 'Save template'}</button>
          {!!profiles?.length && (
            <button onClick={useTemplate} disabled={saving || usingTemplate} title="Save this template, create a portfolio/resume from it, and open the publish screen" className="px-4 py-1.5 rounded-lg bg-primary text-primary-foreground text-xs font-semibold hover:bg-primary/90 disabled:opacity-50 inline-flex items-center gap-1.5"><Icon name="GlobeAltIcon" size={14} /> {usingTemplate ? 'Creating…' : 'Use & publish'}</button>
          )}
        </div>
      </div>
          ...
          {!!profiles?.length && (
            <Field label="Preview with">
              <select
                ...
                className="w-full text-sm rounded-lg border border-border bg-input px-3 py-2 focus-ring"
              >
