...
import { PORTFOLIO_STYLE_TEMPLATES, type CareerTemplate, type CareerView, type GenerateResumeResult, type Resume, type ViewKind } from '../shared/types';
import ErrorBanner from '../shared/ErrorBanner';
import ProfileImportDialog from '../profile/ProfileImportDialog';
import CreateProfileWizard from '../profile/CreateProfileWizard';
...
import { TemplatePickerCard, TemplatePreviewThumb } from '../templates-designer/TemplatePreviewThumb';
import GapAnalysisResult from '../job-match/GapAnalysisResult';
import JobMatchPanel from '../job-match/JobMatchPanel';
import AnalyzerPanel from '../resume/AnalyzerPanel';
import CoverLetterPanel from '../cover-letter/CoverLetterPanel';
import { LIST as RESUME_TEMPLATES } from '../resume/templates';

type Tab = 'profiles' | 'analyzer' | 'generate' | 'match' | 'coverletters' | 'resumes' | 'portfolios' | 'templates';
...
export default function CareerStudioTabs() {
  ...
  useEffect(() => {
    ...
    if (q && ['profiles', 'analyzer', 'generate', 'match', 'coverletters', 'resumes', 'portfolios', 'templates'].includes(q)) setTab(q as Tab);
    void loadProfiles();
  ...
  const TABS: { id: Tab; label: string; icon: string }[] = [
    ...
    { id: 'analyzer', label: 'Analyzer', icon: 'MagnifyingGlassCircleIcon' },
    { id: 'generate', label: 'Generate', icon: 'SparklesIcon' },
    { id: 'match', label: 'Job Matcher', icon: 'ScaleIcon' },
    { id: 'coverletters', label: 'Cover Letters', icon: 'EnvelopeIcon' },
  ...
  return (
    <div className="relative">
      ...
      {tab === 'analyzer' && <AnalyzerPanel profiles={profiles} loaded={profilesLoaded} onGoToProfiles={() => setTab('profiles')} onProfilesChanged={loadProfiles} showToast={showToast} />}
      ...
      {tab === 'match' && <JobMatchPanel profiles={profiles} loaded={profilesLoaded} onGoToProfiles={() => setTab('profiles')} onProfilesChanged={loadProfiles} showToast={showToast} />}
      {tab === 'coverletters' && <CoverLetterPanel profiles={profiles} loaded={profilesLoaded} onGoToProfiles={() => setTab('profiles')} onProfilesChanged={loadProfiles} showToast={showToast} />}
      {tab === 'resumes' && <ViewsTab kind="resume" profiles={profiles} onProfilesChanged={loadProfiles} showToast={showToast} />}
      {tab === 'portfolios' && <ViewsTab kind="portfolio" profiles={profiles} onProfilesChanged={loadProfiles} showToast={showToast} />}
      {tab === 'templates' && <TemplatesTab profiles={profiles} onUse={startCreateView} showToast={showToast} />}
              ...
              {rename?.id === p.id ? (
                <div className="flex items-start gap-3 mb-4 flex-1">
                  ...
                  <input
                    ...
                    className="min-w-0 flex-1 font-heading text-sm font-semibold text-foreground bg-card border border-primary/40 rounded px-1.5 py-1 focus:outline-none"
                  />
                </div>
              ) : (
                <>
                  <div className="flex items-start gap-3 mb-4">
                    <button onClick={() => openProfileEditor(p.id)} className="cs-icon-chip shrink-0"><Icon name="IdentificationIcon" size={18} className="text-primary" /></button>
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1">
                        <button onClick={() => openProfileEditor(p.id)} className="text-left min-w-0 flex-1">
                          <h3 className="font-heading text-sm font-semibold text-foreground truncate">{p.title}</h3>
                        </button>
                        <button onClick={() => setRename({ id: p.id, value: p.title })} className="shrink-0 p-1 rounded text-muted-foreground/70 hover:text-primary hover:bg-primary/5" title="Rename">
                          <Icon name="PencilIcon" size={13} />
                        </button>
                      </div>
                      <p className="text-xs text-muted-foreground mt-0.5">Updated {fmt(p.updated_at)}</p>
                    </div>
                  </div>
                  <button onClick={() => openProfileEditor(p.id)} className="text-left text-xs text-muted-foreground inline-flex items-center gap-1">
                    <Icon name="Bars3BottomLeftIcon" size={14} /> {p.section_count ?? 0} sections
                  </button>
                </>
              )}
...
function ViewsTab({ kind, profiles, onProfilesChanged, showToast }: { kind: ViewKind; profiles: Resume[]; onProfilesChanged: () => void; showToast: (m: string, t?: 'success' | 'error') => void }) {
  const router = useRouter();
  const [views, setViews] = useState<CareerView[]>([]);
  const [loaded, setLoaded] = useState(false);
  const label = kind === 'portfolio' ? 'Portfolio' : 'Resume';
  const fallbackTemplates = (kind === 'portfolio' ? PORTFOLIO_STYLE_TEMPLATES : RESUME_TEMPLATES).map((t) => ({ id: t.id, name: t.name }));
  const [loadedTemplates, setLoadedTemplates] = useState<CareerTemplate[]>([]);
  const templates: Array<{ id: string; name: string; spec?: any }> = loadedTemplates.length ? loadedTemplates : fallbackTemplates;
  ...
  const loadTemplates = useCallback(() => {
    templatesService.list(kind).then(setLoadedTemplates).catch(() => setLoadedTemplates([]));
  }, [kind]);

  useEffect(() => { loadTemplates(); }, [loadTemplates]);

  const onTemplateSaved = (saved: CareerTemplate) => {
    setLoadedTemplates((ts) => (ts.some((x) => x.id === saved.id) ? ts.map((x) => (x.id === saved.id ? saved : x)) : [...ts, saved]));
  ...
  const commitRename = async () => {
    ...
    if (!title.trim()) return;
    try {
      const updated = await viewsService.update(id, { title: title.trim() });
      setViews((vs) => vs.map((x) => (x.id === id ? updated : x)));
    } catch (e: any) {
      showToast(e?.message || 'Rename failed', 'error');
    }
  };

  // Picking a template immediately generates the view — no separate confirm step.
  const quickCreate = async (templateId: string) => {
    if (!profileId) { showToast('Choose a profile first', 'error'); return; }
    setCreatingTemplate(templateId);
    try {
      const v = await viewsService.create({ profileId, kind, title: newTitle.trim() || undefined, template: templateId });
      router.push(`/career/view/${v.id}`);
    } catch (e: any) {
  ...
  return (
    <div>
      ...
      {!loaded ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">{[0, 1, 2].map((i) => <div key={i} className="lab-card p-5 animate-pulse"><div className="h-5 w-2/3 bg-muted rounded mb-4" /><div className="h-3 w-1/2 bg-muted rounded" /></div>)}</div>
      ) : views.length > 0 && (
              ...
              {rename?.id === v.id ? (
                <div className="flex items-start gap-3 mb-4 flex-1">
                  ...
                  <input
                    autoFocus
                    value={rename.value}
                    onChange={(e) => setRename({ id: v.id, value: e.target.value })}
                    onBlur={commitRename}
                    onKeyDown={(e) => { if (e.key === 'Enter') commitRename(); if (e.key === 'Escape') setRename(null); }}
                    className="min-w-0 flex-1 font-heading text-sm font-semibold text-foreground bg-card border border-primary/40 rounded px-1.5 py-1 focus:outline-none"
                  />
                ...
                <>
                  <div className="flex items-start gap-3 mb-4">
                    <button onClick={() => router.push(`/career/view/${v.id}`)} className="cs-icon-chip shrink-0"><Icon name={kind === 'portfolio' ? 'GlobeAltIcon' : 'DocumentTextIcon'} size={18} className="text-primary" /></button>
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1">
                        <button onClick={() => router.push(`/career/view/${v.id}`)} className="text-left min-w-0 flex-1">
                          <h3 className="font-heading text-sm font-semibold text-foreground truncate">{v.title}</h3>
                        </button>
                        <button onClick={() => setRename({ id: v.id, value: v.title })} className="shrink-0 p-1 rounded text-muted-foreground/70 hover:text-primary hover:bg-primary/5" title="Rename">
                          <Icon name="PencilIcon" size={13} />
                        </button>
                      </div>
                      <p className="text-xs text-muted-foreground mt-0.5">Updated {fmt(v.updated_at)}</p>
                    </div>
                  </div>
                  <button onClick={() => router.push(`/career/view/${v.id}`)} className="flex items-center gap-3 text-xs text-muted-foreground text-left">
                    <span className="inline-flex items-center gap-1 capitalize"><Icon name="SwatchIcon" size={13} /> {templates.find((t) => t.id === v.template)?.name || v.template || 'default'}</span>
      ...
      {loaded && profiles.length === 0 ? (
        <div className="lab-card p-12 text-center">
          ...
          <p className="text-sm text-muted-foreground max-w-md mx-auto leading-relaxed">Create a data profile first, then generate {label.toLowerCase()}s from it.</p>
        </div>
      ) : loaded && (
        <div className="lab-card p-5">
          <h3 className="font-heading text-sm font-semibold text-foreground mb-3">Generate a new {label.toLowerCase()}</h3>
          <div className="grid sm:grid-cols-2 gap-4 mb-4">
            <ProfileSelector value={profileId} profiles={profiles} onChange={setProfileId} onProfilesChanged={onProfilesChanged} showToast={showToast} />
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-1.5">Title <span className="font-normal normal-case">(optional)</span></label>
              <input value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder={`${label} name`} className="w-full rounded-lg border border-border bg-input px-3 py-2.5 text-sm text-foreground focus-ring" />
            </div>
          </div>
          <label className="block text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-1.5">Pick a template to generate</label>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2.5">
            {templates.map((t) => (
              <TemplatePickerCard
                ...
                name={t.name}
                spec={(t as any).spec}
                busy={creatingTemplate === t.id}
                disabled={!!creatingTemplate && creatingTemplate !== t.id}
                onClick={() => quickCreate(t.id)}
                onEdit={loadedTemplates.length > 0 ? () => setDesignerTemplate(t as CareerTemplate) : undefined}
              />
            ))}
          </div>
        </div>
      ...
      {designerTemplate && (
        <TemplateDesigner kind={kind} template={designerTemplate} profiles={profiles} initialProfileId={profileId} onClose={() => setDesignerTemplate(null)} onSaved={onTemplateSaved} />
      )}
    </div>
  );
}
  ...
  return (
    <div className="display-card p-3 flex flex-col group">
      <div className="relative h-32 rounded-md border border-border overflow-hidden bg-white mb-2">
        ...
        <TemplatePreviewThumb kind={tpl.kind} spec={tpl.spec} />
...
function TemplatesTab({ profiles, onUse, showToast }: { profiles: Resume[]; onUse: (kind: ViewKind, template: string) => void; showToast: (m: string, t?: 'success' | 'error') => void }) {
  ...
  return (
    <div>
      ...
      {designer && <TemplateDesigner kind={designer.kind} template={designer.template} profiles={profiles} onClose={() => setDesigner(null)} onSaved={onSaved} />}
    </div>
  );
}
...
function CreateViewModal({ kind, initialTemplate, profiles, onClose, onCreated, showToast }: {
  ...
  const [templates, setTemplates] = useState<{ id: string; name: string; spec?: any }[]>(fallback);
      ...
      <div className="relative bg-card border border-border rounded-2xl shadow-2xl w-full max-w-md p-6 animate-fade-in" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2 max-h-64 overflow-y-auto scrollbar-clean pr-1">
              {templates.map((t) => (
                <TemplatePickerCard key={t.id} kind={kind} name={t.name} spec={t.spec} selected={template === t.id} onClick={() => setTemplate(t.id)} />
              ))}
            </div>
          </div>
          <div>
