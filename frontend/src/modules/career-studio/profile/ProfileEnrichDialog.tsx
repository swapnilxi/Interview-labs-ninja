...
import { SECTION_LABELS, type EnrichmentConflict, type EnrichmentKind, type EnrichmentPreview, type ProfileImportSource, type Resume } from '../shared/types';

type Step = 'input' | 'preview';
type Resolution = 'keep' | 'new' | 'suggested';

const KINDS: { id: EnrichmentKind; label: string; icon: string; upload: boolean }[] = [
  { id: 'resume', label: 'Additional resume', icon: 'DocumentDuplicateIcon', upload: true },
...
export default function ProfileEnrichDialog({
  ...
  const buildSource = (): ProfileImportSource | null => {
    if (kind === 'resume') {
      ...
      if (!sourceId) { setError('Pick a resume or profile to pull from, or upload a resume file.'); return null; }
              ...
              {kind === 'resume' ? (
                <div className="space-y-3">
                  <div className="space-y-2">
                    <label className="block text-xs font-medium text-muted-foreground">Pull sections from an existing profile</label>
                    {masters.length === 0 ? (
                      <p className="text-sm text-muted-foreground py-4 text-center">No other resumes or profiles yet — upload a file below instead.</p>
                    ) : (
