...
import type { AnalysisRecord, AnalysisReport } from '../shared/types';

const RESUME_DIMENSIONS: { key: keyof AnalysisReport; label: string }[] = [
...
// Mirrors backend PORTFOLIO_DIMENSIONS (prompt_builder.py) — a portfolio's
// widget content is scored on design/UX/branding, not ATS/grammar.
const PORTFOLIO_DIMENSIONS: { key: keyof AnalysisReport; label: string }[] = [
...
const RESUME_ISSUE_GROUPS: { key: keyof AnalysisReport; label: string }[] = [
...
// Mirrors backend build_portfolio_analyze_prompt's list fields.
const PORTFOLIO_ISSUE_GROUPS: { key: keyof AnalysisReport; label: string }[] = [
  { key: 'missing_sections', label: 'Missing sections' },
...
export default function AnalysisPanel({
  ...
  masterId?: string;
  /** Portfolio widget content is scored on design/UX/branding, not ATS/grammar
   * — switches which dimensions/issue groups render (see PORTFOLIO_DIMENSIONS). */
  ...
  const dimensions = isPortfolio ? PORTFOLIO_DIMENSIONS : RESUME_DIMENSIONS;
  const issueGroups = isPortfolio ? PORTFOLIO_ISSUE_GROUPS : RESUME_ISSUE_GROUPS;
      ...
      <aside className={`fixed top-0 right-0 z-[250] h-full w-[440px] max-w-[95vw] bg-card border-l border-border shadow-xl flex flex-col transition-transform ${open ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex items-center justify-between px-4 h-[56px] border-b border-border flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="font-heading text-sm font-semibold text-foreground">{isPortfolio ? 'AI Portfolio Analysis' : 'AI Resume Analysis'}</span>
          </div>
        ...
        <div className="p-3 border-b border-border flex-shrink-0 space-y-2">
          ...
          <button onClick={run} disabled={running} className="w-full py-2 rounded-lg bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary/90 disabled:opacity-50 inline-flex items-center justify-center gap-2">
            <Icon name="SparklesIcon" size={16} /> {running ? 'Analyzing…' : record ? 'Re-run analysis' : isPortfolio ? 'Analyze portfolio' : 'Analyze resume'}
          </button>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-clean p-3 space-y-4">
          {!report && !running && (
            <div className="text-center py-12">
              ...
              <p className="text-sm text-muted-foreground">{isPortfolio ? 'Run an analysis to get a design/UX score, strengths, and specific fixes.' : 'Run an analysis to get an ATS score, strengths, and specific fixes.'}</p>
              <p className="text-xs text-muted-foreground mt-1">Requires an AI key set in Config.</p>
            </div>
          )}
          {running && !report && <p className="text-sm text-muted-foreground text-center py-12 animate-pulse">{isPortfolio ? 'Scoring your portfolio…' : 'Scoring your resume…'}</p>}

          {report && (
            <>
              <div className="flex gap-2">
                <ScoreBadge label="Overall" value={report.overall_score} />
                <ScoreBadge label={isPortfolio ? 'Recruiter fit' : 'ATS'} value={isPortfolio ? (report as any).recruiter_friendliness_score : report.ats_score} />
              </div>
              ...
              <div className="space-y-2">
                {issueGroups.map((g) => <IssueGroup key={String(g.key)} label={g.label} items={report[g.key] as any} />)}
              </div>
            </>
          )}
        </div>
