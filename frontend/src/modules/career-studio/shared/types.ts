...
/** One section's entry in a section-by-section analysis (distinct from the flat AnalysisRecord above). */
...
/** Result of POST /career/resumes/{id}/analyze/sections — preview-only, not persisted. */
...
export type ProfileImportSource =
  ...
  | { source: 'version'; version_id: string }
...
/** Result of matching a profile against a job — fit/gap report only, no resume generated. */
...
export const SECTION_ICONS: Record<string, string> = {
  personal_info: 'IdentificationIcon',
  ...Object.fromEntries(SECTION_LIBRARY.map((s) => [s.type, s.icon])),
};
...
/** One immutable published snapshot (version history entry). */
...
/** Per-published-view analytics totals, from GET /career/analytics/summary. */
export interface AnalyticsSummaryEntry {
  slug: string;
  master_id: string;
  title: string | null;
  kind: 'portfolio' | 'resume';
...
/** A visitor-submitted recommendation on a published portfolio, held for owner
 * approval — distinct from the owner-authored 'testimonials' widget. */
...
export interface PublicPortfolio {
  ...
  view_count?: number;
  is_public?: boolean;
  /** Present only when reading a specific past snapshot (/p/{slug}/v/{n}). */
  version_number?: number;
  approved_testimonials?: TestimonialSubmission[];
}
...
export interface PublicResume {
  ...
  html: string;
  view_count?: number;
  /** Present only when reading a specific past snapshot (/r/{slug}/v/{n}). */
  version_number?: number;
}
...
export const DEFAULT_PORTFOLIO_SPEC: PortfolioTemplateSpec = {
  font: 'sans', accent: 'violet', layout: 'card', background: 'modern3d',
};
