...
export default function ParetoSidebar() {
  ...
  const fetchTop20 = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/pareto/top20`);
      if (res.ok) {
        ...
        <div className="px-3 pb-3 overflow-y-auto scrollbar-clean">
          {totalTasks === 0 && !loading ? (
            <p className="text-[10px] text-muted-foreground italic py-2 text-center">
              Run 80/20 analysis in any tab to discover your highest-leverage tasks.
            </p>
          ) : (
