        ...
        <div className="flex items-center gap-2 mt-2 flex-wrap">
          ...
          <div className="flex items-center gap-1">
            ...
            <div className="relative">
              ...
              {showParetoPopover && (
                <div className="absolute bottom-full right-0 mb-1 z-50 bg-popover border border-border rounded-lg shadow-lg p-3 min-w-[220px]">
                  ...
                  <div className="space-y-2">
                    <button
                      onClick={async () => {
                        setReanalyzing(true);
                        try {
                          const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/pareto/reanalyze/tasks/${task.id}`, {
                            method: 'POST',
                    ...
                    <label className="w-full flex items-center justify-between px-2 py-1.5 rounded bg-muted/50 hover:bg-muted cursor-pointer transition-smooth">
                      ...
                      <input 
                        ...
                        onChange={async (e) => {
                          ...
                          // Use the generic update flow in backend (requires adding is_top_20 to allowed updates, we'll do this next)
                          const updated = await todoService.updateTask(task.id, { is_top_20: val } as any);
            ...
            <div className="relative">
              <button
                onClick={() => { setShowMoveMenu(!showMoveMenu); setShowDeleteConfirm(false); setShowParetoPopover(false); }}
            ...
            <div className="relative">
              <button
                onClick={() => { setShowDeleteConfirm(!showDeleteConfirm); setShowMoveMenu(false); setShowParetoPopover(false); }}
