...
def seed_defaults(user_id: str) -> None:
    """Populate a user's library with any built-in preset they've never had a
    row for — everything, for a brand-new user; just the newly-added ones
    (e.g. a preset added to template_presets.py after this user's library was
    first seeded), for an existing user. Matched by `source` id, incl.
    soft-deleted rows, so a preset the user explicitly deleted stays deleted
    instead of being resurrected on their next visit."""
    uid = str(user_id)
    conn = _connect()
    try:
        cur = conn.cursor()
        # Scoped by (kind, source) — "modern" is a valid preset id in BOTH the
        # resume and portfolio lists, so a plain source-only check would let
        # an existing resume "Modern" wrongly block seeding portfolio "Modern UI".
        cur.execute("SELECT DISTINCT kind, source FROM career_templates WHERE user_id = ?", (uid,))
        seen = {(r[0], r[1]) for r in cur.fetchall()}
        for kind in ("resume", "portfolio"):
            for p in presets_for(kind):
                if (kind, p["id"]) in seen:
                    continue
                cur.execute(
                    "INSERT INTO career_templates (id, user_id, kind, name, spec_json, source) VALUES (?, ?, ?, ?, ?, ?)",
