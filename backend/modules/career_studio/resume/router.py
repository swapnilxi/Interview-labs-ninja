...
from ..shared.llm import generate_json_array
from ..shared.prompt_builder import build_import_prompt
...
async def export_resume(
    master_id: str,
    format: str = Query("html", pattern="^(html|pdf|docx|markdown)$"),
    template: Optional[str] = None,
    user_id: int = Depends(get_current_user_id),
):
    """Render the resume to a standalone HTML file, a server-generated PDF, a
    Word document, or Markdown.
    ...
    resume's saved template_key for a one-off export (html/pdf only — docx and
    markdown are plain, template-agnostic layouts).
    """
    resume = db.get_resume_tree(str(user_id), master_id)
    if resume is None:
        raise HTTPException(status_code=404, detail="Resume not found")
