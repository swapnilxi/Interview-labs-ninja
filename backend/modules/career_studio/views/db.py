...
_PORTFOLIO_DEFAULTS = {"template": "modern3d", "accent": "violet", "font": "sans", "layout": "stack"}
...
def resolved_to_widgets(sections: list[dict]) -> list[dict]:
    ...
    for s in sections:
        ...
        sid = s["id"]
        if st == "personal_info":
            add(sid + "-hero", "hero", None, {
                "headline": c.get("name", ""), "subheadline": c.get("title", ""), "tagline": c.get("tagline", ""),
                "avatar_url": c.get("avatar_url", ""), "available": bool(c.get("available")),
            })
