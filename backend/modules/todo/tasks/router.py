...
from ..shared.ai_helpers import (
    AISettings,
    call_ai as _call_ai,
    call_vision_ai as _call_vision_ai,
    stream_ai as _stream_ai,
    extract_json_array as _extract_json_array,
    image_to_base64_jpeg as _image_to_base64_jpeg,
)
from .db import (
...
class MoveToPlanRequest(BaseModel):
...
async def move_task_to_quick_endpoint(task_id: int, user_id: int = Depends(get_current_user_id)) -> dict:
    """Move a Smart To-Do task to today's Quick Daily. This is a real move — the
    task is removed from Smart To-Do so it never lingers as a duplicate."""
    task = get_task(task_id, user_id)
    if not task:
        raise HTTPException(status_code=404, detail="Task not found")
...
async def move_task_to_plan_endpoint(
    task_id: int,
    body: MoveToPlanRequest = MoveToPlanRequest(),
    user_id: int = Depends(get_current_user_id),
) -> dict:
    """Move a Smart To-Do task into Plan & Project — either as a node under an
    existing project (body.project_id) or, if none is given, as a brand new
    project. This is a real move — the task is removed from Smart To-Do so it
    never lingers as a duplicate."""
    task = get_task(task_id, user_id)
    if not task:
        raise HTTPException(status_code=404, detail="Task not found")

    from ..projects.db import create_project, create_project_node, get_project

    if body.project_id is not None:
        project = get_project(body.project_id, user_id)
        if not project:
        ...
        delete_task(task_id, user_id)
        return {"status": "moved", "project": project, "node": node}
