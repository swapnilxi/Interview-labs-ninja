...
_NODE_COLUMNS = (
    ...
    "order_index, created_at, pareto_score, is_top_20"
)
...
def update_project_node(node_id: int, **fields) -> Optional[Dict[str, Any]]:
    allowed = {"title", "node_type", "exported_to_smart_todo", "exported_task_id", "exported_to_quick", "order_index", "pareto_score", "is_top_20"}
    updates = {k: v for k, v in fields.items() if k in allowed}
    if not updates:
